...
export default {
    ...
    Mutation:{
        addProduct:async(_:any, args: MutationAddProductArgs) => {
            ...
            try {
                let data  = await new ProductService().createProduct({description, image, name, range});
                return data
              } catch (error) {
                console.log(error);
        ...
        updateProduct: async (_:any, args: MutationUpdateProductArgs) =>{
          ...
          try {
            let data  = await new ProductService().updateProductById({id, description, image, name, range});
            return data
          } catch (error) {
            console.log(error);
            throw new Error("erreur");
